  ...
  return next(req).pipe(
    catchError((error: HttpErrorResponse) => {
      ...
      if (error.status === 401) {
        ...
        toastr.warning("Session Expired", { description: errorMessage });
        ...
        toastr.error("Error", { description: errorMessage });
      } else if (error.status === 0) {
        errorMessage = "Unable to connect to the server. Please check your internet connection.";
        toastr.error("Connection Error", { description: errorMessage });
      } else if (error.status >= 500) {
        errorMessage = "Internal server error. Please try again later.";
        toastr.error("Server Error", { description: errorMessage });
      }
